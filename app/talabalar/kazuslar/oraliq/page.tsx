...
export default function KazuslarPage() {
  ...
  async function loadSubjects() {
    try {
      ...
      const response = await fetch("/api/kazuslar/fanlar?category=ON", {
        method: "GET",
        cache: "no-store",
        credentials: "include",
      });
  ...
  const EXAM_KIND = "ON";
  const EXAM_TITLE = "Oraliq nazorat kazuslari";
  ...
  async function deleteSubject(item: SubjectFolder) {
    ...
    try {
      const response = await fetch(
        `/api/kazuslar/fanlar?id=${encodeURIComponent(item.id)}&category=ON`,
        {
          method: "DELETE",
          credentials: "include",
          headers: {
