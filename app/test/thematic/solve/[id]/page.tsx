...
export default function TestSolvePage() {
  ...
  const [submitDetails, setSubmitDetails] = useState<Record<string, SubmitDetail>>({});
  ...
  async function loadTest() {
    ...
    try {
      const response = await fetch(
        `/api/tests/thematic/${encodeURIComponent(testId)}`,
        {
          method: "GET",
          cache: "no-store",
        }
      ...
      setAttemptLimit(normalizedTest.attemptLimit ?? null);
      setAttemptsUsed(0);
      setAttemptsRemaining(normalizedTest.attemptLimit ?? null);
      setCanAttempt(true);
  ...
  function optionIsCorrect(option: TestOption, questionId?: string) {
    if (questionId) {
      const detail = submitDetails[questionId];
      if (detail?.correctOptionId) {
        return String(detail.correctOptionId) === String(option.id);
      }
    }
  ...
  async function finishTest(auto = false) {
    if (!test || finishLock.current || submitLoading) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/tests/thematic/${encodeURIComponent(test.id)}/submit`,
        {
          method: "POST",
      ...
      if (!response.ok || !data?.success || !data?.result) {
        throw new Error(data?.message || "Natijani tekshirib bo‘lmadi.");
      }

      setServerResult(data.result as ServerResult);

      const detailMap: Record<string, SubmitDetail> = {};
      for (const detail of Array.isArray(data.details) ? data.details : []) {
        if (detail?.questionId) {
          detailMap[String(detail.questionId)] = {
            ...detail,
            questionId: String(detail.questionId),
            selectedOptionId: detail.selectedOptionId == null ? null : String(detail.selectedOptionId),
            correctOptionId: detail.correctOptionId == null ? null : String(detail.correctOptionId),
          };
        }
      }
      ...
      finishLock.current = false;
      setSubmitError(
        err instanceof Error ? err.message : "Natijani tekshirishda xatolik yuz berdi."
      );
      if (!auto) {
        window.alert(
          err instanceof Error ? err.message : "Natijani tekshirishda xatolik yuz berdi."
  ...
  const result = useMemo(() => {
    ...
    if (!test) {
      return {
        ...
        percentage: 0, earnedPoints: 0, totalPoints: 0,
      };
    }
    ...
    for (const question of test.questions) {
      ...
      totalPoints += points;
      if (!answers[question.id]) unanswered++;
    }

    return {
      total: test.questions.length,
      correct: 0,
      incorrect: test.questions.length - unanswered,
      unanswered,
      percentage: 0,
      earnedPoints: 0,
      totalPoints,
    };
  }, [test, answers, serverResult]);

  /* =====================================================
     NATIJA SERVERDA TEKSHIRILADI
  ===================================================== */
                          ...
                          <div>
                            ...
                            {(() => {
                              const correctOption =
                                question.options?.find(
                                  (option) =>
                                    optionIsCorrect(option, question.id)
                                );
